...
//        master.setSeatsfee(table.getSeatfee()*nums);//餐位费，
        ...
        master.setMemo(memo);
        int count = orderMasterMapper.insert(master);
        table.setFlag("2");
    ...
    public ResponseJson editNums() throws AjaxOperationFailException {
        ...
        master.setNums(nums);
        int count = orderMasterMapper.updateByPrimaryKey(master);
        ResponseJson resp = new ResponseJson();
        if (count > 0) {
    ...
    public ResponseJson hetai() throws AjaxOperationFailException {
        ...
        if (flag.equals("1")) {//合台
            ...
            master.setFlag("1");
            int count = orderMasterMapper.updateByPrimaryKey(master);//直接修改
            resp.setMsg("合台成功!");
        } else {//联台
            String tablesides = "";
            if (master.getFlag().equals("0")) {
                ...
                for (String id : master.getTableids().split(",")) {
                    ...
                    orderMasterMapper.updateByPrimaryKey(master1);
                }
            }
            //追加生成
            for (String id : idarray) {
                OrderMaster master1 = orderMasterMapper.selectByPrimaryKey(orderid);
                Tables table = tablesMapper.selectByPrimaryKey(Integer.valueOf(id));
//                OrderMaster master1 = new OrderMaster();
                ...
                master1.setIsall("0");
                master1.setStatus("0");
                orderMasterMapper.insert(master1);
            }
            resp.setMsg("连台成功!");
        }
        return resp;
    ...
    public ResponseJson cancel() throws AjaxOperationFailException {
        ...
        if (master.getFlag().equals("0")) {//独台
            ...
            if (ids.length == 2) {//2台连台情况下，结账一台，另一台自动变独台
                ...
                OrderMaster anotherorder = orderMasterMapper.selectByPrimaryKey(anotherid);
                anotherorder.setTableids(null);
                anotherorder.setFlag("0");
                orderMasterMapper.updateByPrimaryKey(anotherorder);
            }
        ...
        orderMasterMapper.deleteByPrimaryKey(orderid);//删除当前订单
        ResponseJson resp = new ResponseJson();
        resp.setMsg("取消成功!");
        return resp;
    }
...
//            int i = orderSlaveMapper.insert1(slave);
        ...
        orderMasterMapper.updateByPrimaryKey(master);//点菜后对应金额增加
        if (tag == 1) {
            //添加打印信息
            addPrintMessage(master, orderSlaves, "1");
        }
     ...
     */
    public void addPrintMessage(OrderMaster master, List<OrderSlave> orderSlaves, String type) {
        ...
        orderPrintMaster.setType(type);//类型（0：点菜  1：加菜  2：退菜  3：结账）
...
//                orderSlaveMapper.save(orderSlaveInfo);
            ...
            orderPrintSlave.setRefundmoney(orderSlaveInfo.getRealamount());
...
//        List<OrderSlave> list = orderSlaveMapper.selectOrderSlaveByOrderId(slave);//查询当前订单的所有点菜
        List<OrderSlave> list = orderSlaveMapper.selectOrderSlaveAllByOrderId(slave.getOrderid() + "");//查询当前订单的所有点菜
        /*
...
//        Double discountamount = 0.0;  //折扣金额
        for (OrderSlave os : list) {
            /*if (!"2".equals(os.getStatus())){
            }*/
            if ("1".equals(isall)) {//整单打折
                os.setRealamount(NumberUtil.mul(os.getOrderamount(), discountratio));
                orderSlaveMapper.updateByPrimaryKey(os);
            } else if ("1".equals(os.getState())) {//可以打折的菜品
                os.setRealamount(NumberUtil.mul(os.getOrderamount(), discountratio));
                orderSlaveMapper.updateByPrimaryKey(os);
            }
            Double realamount = os.getRealamount();
            if (realamount == null) {
                realamount = 0.0;
            }
            total = NumberUtil.add(realamount, total);
        }
        //计算其他费用（餐位费+服务费）
        Stores stores = storesMapper.selectByPrimaryKey(employeeService.getLoginUser().getStoreid());
        //统计餐台服务费
...
//        if ("1".equals(isall)) {//整单打折
//            total = NumberUtil.add(total, Double.valueOf(NumberUtil.mul(total, discountratio)));
//        } else {//非整单打折
//        }
    ...
    public ResponseJson getOrderDiscountAmount(Integer orderId, Double discountratio, String isall, Integer settleType) throws AjaxOperationFailException {
        ...
        OrderMaster master = orderMasterMapper.selectByPrimaryKey(orderId);//获取订单信息
     ...
     * @param settleType    结算类型，1单台，2，连台
     * @param master        订单
     * @throws AjaxOperationFailException return
     */
...
//            master.setIsmultiple(2);//连台结算
            ...
            for (OrderMaster os : list) {
                ...
                totalmoney = NumberUtil.add(totalmoney, getOrderDiscountMoney(os, 1.0, isall));
            }
        }
        //获取已付金额
        Double count = payRecordsMapper.selectCountByOrderid(master.getOrderid(), "");
        if (count == null) {
            count = 0.0;
        }

//        master.setTotalmoney(NumberUtil.sub(master.getTotalmoney(), discountAmount));

//        master.setDiscountid(discountId);//折扣id
        master.setDiscountratio(discountratio);//折扣比例
        master.setTotalmoney(totalmoney);
        master.setDiscountamount(NumberUtil.sub(totalmoney, total));//优惠金额

        Double discountAmount = NumberUtil.add(master.getSeatsfee(), master.getDiscountamount());
        master.setPayamount(NumberUtil.sub(master.getTotalmoney(), discountAmount));
        Double alsopay = NumberUtil.sub(master.getTotalmoney(), count, discountAmount).doubleValue();
        if (alsopay <= 0.0) {
            alsopay = 0.0;
        }
        master.setAlsoypay(alsopay);
...
//        OrderMaster master = orderMasterMapper.selectByPrimaryKey(orderid);
        ...
        if (discountratio == null || discountratio == 0.00) {
            discountratio = 1.0;
        }
...
//        master.setTablefee(tablesMapper.selectByPrimaryKey(master.getTableid()).getFixedfee());
...
//        master.setIsmultiple(2);//合台结算
        List<OrderMaster> masterList = orderMasterMapper.findOrderMasterByType(master);
        if (!StringUtil.isListNull(masterList)) {//表示多台已经在结算中
        ...
        for (OrderMaster os : list) {
            ...
            if (!orderIsSettle(os) && master.getDiscountratio() != null && master.getDiscountratio() != 0) {//没结过账,打过折
                //清空折扣
                os.setDiscountid(0);
                os.setIsall("0");
                ...
                orderMasterMapper.updateByPrimaryKey(os);
                //清空折扣
                OrderSlave slave = new OrderSlave();
                slave.setOrderid(master.getOrderid());
...
//                    s.setState("0");//0不打折，1打折
                    orderSlaveMapper.updateByPrimaryKey(s);
                }
            }
...
//                OrderMaster master1 = orderMasterMapper.selectByPrimaryKey(Integer.valueOf(list.get(i).get("orderid")+""));
...
//                top1.put("tablename",tablesMapper.selectByPrimaryKey(orderMasterMapper.selectByPrimaryKey(Integer.valueOf(id)).getTableid()).getCode());
...
//                Map top1 = new HashMap();
//                List<Map> dishes = null;
//                top1.put("tablename",tablesMapper.selectByPrimaryKey(Integer.valueOf(id)).getCode());
...
//    }
    ...
    public ResponseJson useAccount1() throws AjaxOperationFailException {
        ...
        String paytype = HttpRequestParamter.getString("paytype");
        ...
        Integer employeeid = employeeService.getLoginUser().getEmployeeid();
        ...
        if (tag == 2) {//连台结账
            ...
            master.setIsmultiple(2);//合台结算
            List<OrderMaster> masterList = orderMasterMapper.findOrderMasterByType(master);
            if (!StringUtil.isListNull(masterList)) {//表示多台已经在结算中,抽取出正在结算的那条订单作为结账
                for (OrderMaster os : masterList) {
                    if ("2".equals(os.getIsmultiple())) {
                        master = os;
                ...
                masterList = orderMasterMapper.selectBytablesids(master);
                Double totalMoney = 0.0;        //连台总金额
                Double tablefee = 0.0;          //服务费
                Double amountMoney = 0.0;       //点菜金额

                if (!StringUtil.isListNull(masterList)) {
                    for (OrderMaster orderMaster : masterList) {
                        tablefee = NumberUtil.add(orderMaster.getTablefee(), tablefee);
                        amountMoney = NumberUtil.add(orderMaster.getAmountmoney(), amountMoney);
                    }
                }
                master.setTablefee(tablefee);
                master.setAmountmoney(amountMoney);
        ...
        if (discountamount1 > employees.getDiscountamount()) {
            throw new AjaxOperationFailException("你的免单上限为！" + employees.getDiscountamount() + "元，如要优惠更多请联系管理员!");
        }
...
//        }
        ...
        ResponseJson resp = new ResponseJson();
        if (!orderIsSettle(master) && !master.getFlag().equals("2")) {//说明没有支付过，可以修改比例和金额
            //获取优惠总金额

            master.setSeatsfee(discountamount1);
            if (discountratio == 1.0) {
                master.setDiscountamount(0.0);
            } else {
                master.setDiscountamount(NumberUtil.mul(master.getTotalmoney(), discountratio));
            }
            master.setAlsoypay(NumberUtil.sub(master.getTotalmoney(), master.getDiscountamount(), discountamount1).doubleValue());
//            master.setPayamount(NumberUtil.sub(master.getTotalmoney(), master.getDiscountamount(), discountamount1).doubleValue());
            master.setPayamount(NumberUtil.sub(master.getTotalmoney(), master.getDiscountamount()));
            if (master.getAlsoypay() <= 0.0) {
        ...
        if (customerid != 0) {
            ...
            if (djflag == 1) {//定金抵扣
                ...
                PayRecords records = payRecordsMapper.selectByPrimaryKey(Integer.valueOf(map.get("payid") + ""));
                if (records == null) {
                ...
                payamount = NumberUtil.sub(payamount, records.getPayamount());
//                orderMasterMapper.updateByPrimaryKey(master);
...
//                    }
        ...
        master.setCustomerid(customerid);
        //优惠券抵扣
        if (quanid != 0 && "9".equals(paytype)) {
            Paytype quan = paytypeMapper.selectByPrimaryKey(quanid);
            if (quan == null) {
            ...
            payRecordsnew.setPayamount(quanMoney);
            payRecordsnew.setPaynum(quannum);
            createPayInfo(payRecordsnew, customerid, orderid);
            //当抵扣完后就直接返回了
            if (master.getAlsoypay() <= 0) {
//                orderMasterMapper.updateByPrimaryKey(master);
...
//            orderMasterMapper.updateByPrimaryKey(master);
        } else if ("1".equals(paytype) || "2".equals(paytype)) {//微信
            payRecordsnew.setPayamount(payamount);
            //支付
            Map map = pay(payRecordsnew.getPayno(), yuanTofen(payRecordsnew.getPayamount()), code);
            payRecordsnew.setPaytype(Integer.parseInt(paytype));//付详细方式：1：微信  2：支付宝  3：现金  4：银行卡 5：余额 6 :签单 7：三倍充值免单 8：霸王餐 9：其他（优惠券）
...
//            master = updateMasterAndTables(master,tag);
    ...
    public void createPayInfo(PayRecords payRecordsnew, Integer customerid, Integer orderid) {
        Employees employees = employeeService.getLoginUser();
        if (payRecordsnew.getPaynum() == null) {
            payRecordsnew.setPaynum(1);
        }
        ...
        payRecordsMapper.insert(payRecordsnew);
        OrderPays orderPays = new OrderPays();
        ...
        if (tag == 1) {//单台结账
            ...
            if (ids.length > 2) {//多台连台，结账一台，另外的继续是连台
                ...
                for (Integer id : listid) {
                    ...
                    orderMasterMapper.updateByPrimaryKey(order);
                }
            }
            master.setFlag("0");
...
//            for (String id : ids) {
            for (int i = 0; i < ids.length; i++) {
                ...
                if (!master.getOrderid().equals(master1.getOrderid())) {
...
//            orderMasterMapper.updateByPrimaryKey(master);
    ...
    public ResponseJson dishesChangeOrder() throws AjaxOperationFailException {
        ...
        OrderMaster master = orderMasterMapper.selectByPrimaryKey(oldorderid);
        OrderMaster master1 = orderMasterMapper.selectByPrimaryKey(neworderid);
        if (master == null) {
            throw new AjaxOperationFailException("订单信息不存在");
        }
        if (master1 == null) {
        ...
        for (String id : array) {
            OrderSlave slave = orderSlaveMapper.selectByPrimaryKey(Integer.valueOf(id));
            ...
            orderSlaveMapper.updateByPrimaryKey(slave);
        }
        orderMasterMapper.updateByPrimaryKey(master);
        orderMasterMapper.updateByPrimaryKey(master1);
        ResponseJson resp = new ResponseJson();
...
//        master.setPayamount(NumberUtil.sub(master.getPayamount(),discountamount));
        ...
        if (overtype.equals("1")) {//签单
            ...
            master.setDiscountratio(discountratio == 0.0 ? 1.0 : discountratio);
            ...
            master = updateMasterAndTables(master, tag);
            master.setStatus("4");
        } else if (overtype.equals("2")) {//三倍充值免单
            if (customerid == 0) {
            ...
            //释放餐桌占用
            master = updateMasterAndTables(master, tag);
        } else if (overtype.equals("3")) {
            //释放餐桌占用
            master.setEmployee1id(employeeService.getLoginUser().getEmployeeid());
            master = updateMasterAndTables(master, tag);
        } else if ("4".equals(overtype)) {
//            Integer employeeid = employeeService.getLoginUser().getEmployeeid();
//            if (employeeid == null || employeeid == 0) {
//                throw new AjaxOperationFailException("该员工信息不存在");
//            }
//            Employees employees = employeesMapper.selectByPrimaryKey(employeeid);
//            if (employees == null) {
//                throw new AjaxOperationFailException("该员工信息不存在");
//            }
//            Double employeesDiscountamount = employees.getDiscountamount();
//            if (employeesDiscountamount == null || employeesDiscountamount == 0.0) {
//                throw new AjaxOperationFailException("您没有优惠权限");
//            }
//            if (discountamount > employeesDiscountamount) {
//                throw new AjaxOperationFailException("超过可退款最大金额");
//            }
//            if (orderMasterMapper.)
        }
        orderMasterMapper.updateByPrimaryKey(master);
        createOrderdetailinfo(orderid);
    ...
    public ResponseJson dishesDel() throws AjaxOperationFailException {
        ...
        for (String id : idarray) {
            ...
            master = orderMasterMapper.selectByPrimaryKey(slave.getOrderid());
...
//            master.setAmountmoney(master.getAmountmoney()-(count*slave.getPrice()));
            ...
            int i = orderSlaveMapper.insert(slavenew);//保存新的记录
        ...
        //添加打印信息
        addPrintMessage(master, orderSlaves, "2");
        ResponseJson resp = new ResponseJson();
        resp.setMsg("删除成功！");
        return resp;
    }
    ...
    public void createSignPay(String[] ids, PayRecords records) {
        if (records.getPaynum() == null) {
            records.setPaynum(0);
        }
        Integer payid = payRecordsMapper.insert(records);
        for (String id : ids) {
            OrderMaster master = orderMasterMapper.selectByPrimaryKey(Integer.valueOf(id));
    ...
    public Integer getCusIdByOrderSign(String[] ids) throws AjaxOperationFailException {
        ...
        for (int i = 0; i < ids.length; i++) {
            OrderMaster master = orderMasterMapper.selectByPrimaryKey(Integer.valueOf(ids[i]));
            if (i == 0) {
    ...
    public ResponseJson fanjieThird() throws AjaxOperationFailException {
        ...
        if ("3".equals(master.getStatus())) {//正常结算，才能退款，
            ...
            for (PayRecords pr : prList) {
                ...
                payRecordsMapper.updateByPrimaryKey(pr);//修改支付记录
                if (alsoypay == 0) break;
            }
            //其他退款
            ...
            for (PayRecords pr : prList) {
                ...
                if (pr.getFlag().equals("2")) {//订金
                    ...
                    orderPaysMapper.deleteByPrimaryKey(pays.getId());
...
//        master.setStatus("6");
    ...
    public ResponseJson changeDishesAmount() throws AjaxOperationFailException {
        ...
        OrderSlave slave = orderSlaveMapper.selectByPrimaryKey(id);
        if (slave == null) {
        ...
        if (oldcount > amount) {
            ...
            orderSlaveMapper.insert(slave1);
        }
        if (dishe.getIsGoods().equals("1")) {
    ...
    public ResponseJson printDishes() throws AjaxOperationFailException {
        ...
        String[] idarray = ids.split(",");
        OrderSlave slave = orderSlaveMapper.selectByPrimaryKey(Integer.valueOf(idarray[0]));
        OrderMaster master = orderMasterMapper.selectByPrimaryKey(slave.getOrderid());
        //添加打印信息
        OrderPrintMaster orderPrintMaster = new OrderPrintMaster();
        orderPrintMaster.setCompanyid(master.getCompanyid());
        orderPrintMaster.setStoreid(master.getStoreid());
        ...
        OrderPrintSlave orderPrintSlave;
        for (String id : idarray) {
            OrderSlave orderSlaveInfo = orderSlaveMapper.selectByPrimaryKey(Integer.valueOf(id));
            orderPrintSlave = new OrderPrintSlave();
            orderPrintSlave.setOrderid(orderPrintMaster.getPrintid());
            orderPrintSlave.setTablesid(master.getTableid());
            orderPrintSlave.setDisheid(orderSlaveInfo.getDisheid());
...
//                OrderMaster master = orderMasterMapper.selectByPrimaryKey(Integer.valueOf(orderid1));
//                master.setStatus("2");
//                orderMasterMapper.updateByPrimaryKey(master);
//            }
//        }else {
...
//        }
    ...
    public ResponseJson refund() throws AjaxOperationFailException {
        ...
        String dishes = HttpRequestParamter.getString("dishes", true);
        if (orderid == 0) {
            throw new AjaxOperationFailException("订单id不能为空");
        }
        if (StringUtil.isNull(dishes)) {
            throw new AjaxOperationFailException("退菜菜品不能为空");
        }
        OrderMaster orderMaster = orderMasterMapper.selectByPrimaryKey(orderid);
        if (orderMaster == null) {
            throw new AjaxOperationFailException("该订单信息不存在");
        }
        long todayDate = DateUtil.toDate(DateUtil.format(new Date(), "yyyy-MM-dd"), "yyyy-MM-dd").getTime();
        long nextDate = todayDate + 24 * 60 * 60 * 1000;
        long intime = orderMaster.getOrdertime().getTime();
        if (intime < todayDate || intime > nextDate) {
            throw new AjaxOperationFailException("已超过该订单可退款时间");
        }
        //状态：0-开台 1-部分结账 3-全部结账 4-签单未支付 5-签单已支付
        String status = orderMaster.getStatus();
        if (!"3".equals(status) && !"4".equals(status) && !"5".equals(status)) {
            throw new AjaxOperationFailException("该订单未结算，不支持退款");
        }
        ...
        OrderFlag2Records flag2Records;
        String otherorderid;
        StringBuffer msg = new StringBuffer("where 1=1 ");
        if (orderMaster.getIsmultiple() == 2) {
            flag2Records = orderFlag2RecordsMapper.selectByOrderid(orderid);
            if (flag2Records != null) {
                otherorderid = flag2Records.getOtherorderid();
                msg.append(" and orderid in (")
                        .append(otherorderid.substring(0, otherorderid.length() - 1))
                        .append(")");
            }
        } else {
            msg.append(" and orderid=").append(orderid);
        }
        msg.append(" and flag in (0,2)");
        //获取餐桌点菜列表信息（菜品id，菜品数量，菜品结账金额）
        List<OrderSlave> orderSlaveList = orderSlaveMapper.
                selectOrderDishesCountByOrderId(msg.toString());
        if (StringUtil.isListNull(orderSlaveList)) {
            throw new AjaxOperationFailException("该餐桌未点菜，不能进行退款");
        }
        Double totalMoney = 0.0;   //存储退款总金额
        ...
        String name;
        String memo;
        String isall = orderMaster.getIsall();
        Double discountratio = orderMaster.getDiscountratio();
        if (discountratio == null || discountratio == 0.0) {
            discountratio = 1.0;
        }
        boolean flag;
        Dishes dishesinfo;
        List<OrderSlave> refundDishesList = new ArrayList<OrderSlave>();
        for (int i = 0; i < jsonArray.size(); i++) {
            jsonObject = jsonArray.getJSONObject(i);
            dishesid = jsonObject.getIntValue("dishesid");
            count = jsonObject.getIntValue("count");
            memo = jsonObject.get("memo") + "";
            flag = false;
            dishesinfo = dishesMapper.selectByPrimaryKey(dishesid);
            if (dishesinfo == null) {
                throw new AjaxOperationFailException("该菜品不存在");
            }
            for (OrderSlave orderSlave : orderSlaveList) {
                slaveDishesid = orderSlave.getDisheid();
                amount = orderSlave.getAmount();
                name = orderSlave.getName();
                if (dishesid == slaveDishesid) {
                    if (count > amount) {
                        throw new AjaxOperationFailException(name + "剩余数量为" + amount + "，超过退菜数量");
                    }
                    //添加退菜菜品记录信息
                    //合台处理方式为：直接记录在当前订单点菜信息记录，不用处理是否为负数，直接计算总数即可
                    orderSlave.setAmount(-count);
                    orderSlave.setOrderamount(NumberUtil.mul(count, (Number) orderSlave.getPrice()).doubleValue());
                    if ("1".equals(isall)) {
                        orderSlave.setRealamount(NumberUtil.mul(orderSlave.getOrderamount(), discountratio));
                    } else if ("1".equals(orderSlave.getState())) {
                        orderSlave.setRealamount(NumberUtil.mul(orderSlave.getOrderamount(), discountratio));
                    }
                    orderSlave.setStatus("2");
                    orderSlave.setFlag("2");
                    orderSlave.setMemo(memo);
                    orderSlave.setEmployeeid(employeeService.getLoginUser().getEmployeeid());
                    orderSlave.setIntime(new Date());

                    totalMoney = NumberUtil.add(orderSlave.getRealamount(), totalMoney);
                    flag = true;
                    refundDishesList.add(orderSlave);
                    break;
                }
            }
            if (!flag) {
                throw new AjaxOperationFailException("您未点【" + dishesinfo.getName() + "】，不能退款");
            }
        }

        List<OrderPays> list = orderPaysMapper.selectByOrderid(orderid);
        String payids = "";
        if (!StringUtil.isListNull(list)) {
            for (OrderPays orderPays : list) {
                if (StringUtil.isNull(payids))
                    payids += orderPays.getPayid();
                else
                    payids += "," + orderPays.getPayid();
            }
        }
        ...
        //获取已付金额
        Double paycount = payRecordsMapper.selectCountByOrderid(orderMaster.getOrderid(), " and paytype in('1','2','3','4','5')");
//        if (paycount == null || paycount == 0.0) {
//            throw new AjaxOperationFailException("该订单支付金额为0，不能退款");
//        }
        totalMoney = Math.abs(totalMoney);

        if (paycount < Math.abs(totalMoney)) {
            totalMoney = paycount;
        }
        paycount = NumberUtil.sub(paycount, totalMoney);
        //获取优惠总金额
//        Double discountAmount = NumberUtil.add(orderMaster.getSeatsfee(), orderMaster.getDiscountamount());
        orderMaster.setAmountmoney(NumberUtil.sub(orderMaster.getAmountmoney(), totalMoney));
        orderMaster.setTotalmoney(NumberUtil.sub(orderMaster.getTotalmoney(), totalMoney));
        orderMaster.setPayamount(NumberUtil.sub(orderMaster.getTotalmoney(), orderMaster.getSeatsfee()));
        orderMaster.setAlsoypay(NumberUtil.sub(orderMaster.getTotalmoney(), paycount));
        orderMasterMapper.updateByPrimaryKey(orderMaster);
        List<PayRecords> payRecordsList = payRecordsMapper.selectListByPaydis(payids);


        for (OrderSlave orderSlave : refundDishesList) {
            orderSlaveMapper.insert(orderSlave);
        }
        //添加消费记录信息
        PayRecords records = new PayRecords();
        records.setPayno(StringUtil.getUUid());
        records.setCompanyid(employeeService.getLoginUser().getCompanyid());
        records.setStoreid(employeeService.getLoginUser().getStoreid());
        records.setCustomerid(0);
        records.setMemo("退款");
        records.setEmployeeid(employeeService.getLoginUser().getEmployeeid());
        records.setPayamount(-totalMoney);
        records.setPaytime(new Date());
        records.setFlag("3");
        records.setOverflag("0");
        records.setReturnamount(-totalMoney);
        records.setReturntime(new Date());
        records.setPaytypedesc("");


        if (!StringUtil.isListNull(payRecordsList)) {
            if (payRecordsList.size() == 1) {
                PayRecords payRecords = payRecordsList.get(0);
                records.setPaytype(payRecords.getPaytype());

                if ("1".equals(payRecords.getPaytype()) || "2".equals(payRecords.getPaytype())) {
                    //调用退款接口
                    records = payRefund(records.getPayno(), totalMoney + "", payRecords.getReturnno(), records);
                } else {
                    records.setStatus("3");
                    if (payRecords.getPaytype() == 5) {
                        CustomersAccounts accounts = accountsMapper.selectByCusId(orderMaster.getCustomerid());
                        if (accounts != null) {
                            Double groupbalance = accounts.getGroupbalance();
                            groupbalance = NumberUtil.add(groupbalance, totalMoney);

                            accountsMapper.updateByPrimaryKey(accounts);
                            CustomersLogs logs = new CustomersLogs();
                            logs.setCompanyid(employeeService.getLoginUser().getCompanyid());
                            logs.setStoreid(employeeService.getLoginUser().getStoreid());
                            logs.setCustomerid(orderMaster.getCustomerid());
                            logs.setYueamount(accounts.getGroupbalance());
                            logs.setOpeation("退款");
                            logs.setAmount(totalMoney);
                            logs.setMemo("退款" + totalMoney + "元");
                            logs.setIntime(new Date());
                            logs.setEmployeeid(employeeService.getLoginUser().getEmployeeid());
                            logsMapper.insert(logs);
                            //发送短信
                            Customers cus = customersMapper.selectByPrimaryKey(orderMaster.getCustomerid());
                            smsService.sendMessage(cus.getPhone(), "您好，" + cus.getName() + "，本次退款" + totalMoney + "元，现有余额" + groupbalance + "元。");
                        }
                    }
                }
            } else {  //退现金
        ...
        payRecordsMapper.insert(records);

        OrderPays orderPays = new OrderPays();
        orderPays.setOrderid(orderid);
        orderPays.setPayid(records.getPayid());
        orderPays.setOktime(new Date());
        orderPays.setStatus("1");
        orderPaysMapper.insert1(orderPays);
        //添加打印信息
        addPrintMessage(orderMaster, refundDishesList, "5");

        return new ResponseJson("退款成功");
    }

    /**
     * 退款
     */
    public static PayRecords payRefund(String ordernumber, String returnmoney, String returnno, PayRecords records) throws AjaxOperationFailException {
        HttpProxy hp = new HttpProxy("https://api.shouqianba.com");
        String terminal_sn = "100009200009978067";
        String terminal_key = "ac158be92e8b4989422182197fcc2798";
        Map<String, Object> result = hp.refund(terminal_sn, terminal_key, ordernumber,
                "2908947314912861", returnmoney, returnno, "gyxg001");   //退款
    ...
    public ResponseJson useAccount() throws AjaxOperationFailException {
        ...
        if (StringUtil.isNull(passwd) && "6".equals(paytype)) {
            throw new AjaxOperationFailException("支付密码不能为空");
        }
        ...
        if ("9".equals(paytype) && quanid == 0) {
            throw new AjaxOperationFailException("优惠券id不能为空");
        }
        ...
        List<OrderSlave> orderSlaveList = new ArrayList<OrderSlave>();
        if (tag == 2) {//连台结账
            //判断是否是第一次结账
            OrderFlag2Records orderFlag2Records = orderFlag2RecordsMapper.selectInfoByOrderid(orderid);
            if (orderFlag2Records != null) {  //不是第一次结账
                String otherid = orderFlag2Records.getOtherorderid();
                if (!StringUtil.isNull(otherid)) {
                    otherid = otherid.substring(0, otherid.length() - 1);
                }
                orderMasterList = orderMasterMapper.selectByOrderIds(otherid);
                master = orderMasterMapper.selectByPrimaryKey(orderFlag2Records.getOrderid());
            } else {
                //获取连台信息
                orderMasterList = orderMasterMapper.selectBytablesids(master);
            }
            master.setIsmultiple(2); //设置为连台

            //获取已付金额
            Double count = 0.0;
            if (orderFlag2Records != null) {
                count = payRecordsMapper.selectCountByOrderid(orderFlag2Records.getOrderid(), "");
            }
            if (count == null) {
                count = 0.0;
            }
            ...
            Double paymoney = 0.0; // 总金额
            Double tratio = master.getDiscountratio();  //折扣率
            Double discountMoney = master.getDiscountamount();  //免单金额
            Double totalTablefee = 0.0;  //餐桌服务费
            Double tablefee = 0.0;
            //计算其他费用（餐位费+服务费）
            //统计餐台服务费
            String orderids = "";
            if (!StringUtil.isListNull(orderMasterList)) {
                for (OrderMaster orderMaster : orderMasterList) {
                    if (StringUtil.isNull(orderids)) {
                        orderids = orderMaster.getOrderid() + "";
                    } else {
                        orderids += "," + orderMaster.getOrderid();
                    }
                    tablefee = tablesMapper.selectByPrimaryKey(master.getTableid()).getFixedfee();
                    totalTablefee = NumberUtil.add(tablefee, totalTablefee);
                }
                Stores stores = storesMapper.selectByPrimaryKey(employeeService.getLoginUser().getStoreid());
                if ("0".equals(stores.getServicer_flag())) {//不收收取服务费
                    totalTablefee = 0.00;
                }
                orderSlaveList = orderSlaveMapper.selectOrderSlaveAllByOrderId(orderids);
                if (!StringUtil.isListNull(orderMasterList)) {
                    if (orderFlag2Records != null) {
                        if (discountratio != null && discountratio != 1.0) {
                            tratio = discountratio;
                        }
                        if (discountamount1 != null && discountamount1 != 0.0) {
                            discountMoney = discountamount1;
                        }
                    } else {
                        if (tratio == null || tratio == 0.0) {
                            tratio = 1.0;
                        }
                    ...
                    master.setDiscountratio(tratio);
                    master.setSeatsfee(discountMoney);
                    master.setIsall(isall);
                    for (OrderSlave orderSlave : orderSlaveList) {
                        if ("1".equals(master.getIsall())) {
                            orderSlave.setRealamount(NumberUtil.mul(orderSlave.getOrderamount(), tratio));
                        } else if ("1".equals(orderSlave.getState())) {
                            orderSlave.setRealamount(NumberUtil.mul(orderSlave.getOrderamount(), tratio));
                        }
                        paymoney = NumberUtil.add(orderSlave.getRealamount(), paymoney);
                        totalMoney = NumberUtil.add(orderSlave.getOrderamount(), totalMoney);
                    }
                }

            }
            totalMoney = NumberUtil.add(tablefee, totalMoney);
            paymoney = NumberUtil.add(tablefee, paymoney);
            master.setDiscountamount(NumberUtil.sub(totalMoney, paymoney));

            paymoney = NumberUtil.sub(paymoney, discountMoney);
            master.setTotalmoney(totalMoney);
            master.setPayamount(paymoney);
            master.setAlsoypay(NumberUtil.sub(paymoney, count));
            if (master.getAlsoypay() <= 0.0) { //表示已支付完成
                //修改订单信息
                updateMasterOverAndTables(master, 2, orderSlaveList);
                //修改其余餐桌信息
                updateLinkTableOver(orderMasterList, master);

                return new ResponseJson("结账成功", 1);
            }
            if (orderFlag2Records == null) {
                orderFlag2Records = new OrderFlag2Records();
                orderFlag2Records.setOrderid(orderid);
                orderFlag2Records.setOtherorderid(orderids);
                orderFlag2RecordsMapper.insert(orderFlag2Records);
            }
        } else {
            //判断是否是第一次结账
            if (!orderIsSettle(master)) {  //若为第一次结账，判断是否有连台信息
                List<OrderMaster> orderMasters = orderMasterMapper.selectBytablesids(master);
                //修改连台信息
                String tableids = "";
                String othertables = master.getTableids();
                master.setTableids("");
                for (OrderMaster orderMaster: orderMasters) {
                    if (!orderMaster.getOrderid().equals(master.getOrderid())) {
                        if (StringUtil.isNull(tableids)) {
                            tableids = "" + orderMaster.getTableid();
                        } else {
                            tableids += "," + orderMaster.getTableid();
                        }
                    }
                ...
                for (OrderMaster orderMaster: orderMasters) {
                    if (!StringUtil.isNull(orderMaster.getTableids()) &&
                            orderMaster.getTableids().equals(othertables) &&
                            !orderMaster.getOrderid().equals(master.getOrderid())) {
                        orderMaster.setTableids(tableids);
                        orderMasterMapper.updateByPrimaryKey(orderMaster);
                    }
                }
            ...
            orderSlaveList = orderSlaveMapper.selectOrderSlaveAllByOrderId(orderid + "");
            Double totalMoney = 0.0; // 总金额
            Double paymoney = 0.0; // 总金额
            Double tratio = master.getDiscountratio();  //折扣率
            Double discountMoney = master.getDiscountamount();  //免单金额
            ...
            master.setAlsoypay(NumberUtil.sub(paymoney, count));
            if (master.getAlsoypay() <= 0.0) { //表示已支付完成
                updateMasterOverAndTables(master, 1, orderSlaveList);
                return new ResponseJson("结账成功", 1);
            }
        }
        ...
        if (customerid != 0) {
            ...
            if (djflag == 1) {
                ...
                orderPays.setPayid(records.getPayid());
                orderPaysMapper.insert1(orderPays);
                master.setAlsoypay(NumberUtil.sub(master.getAlsoypay(), records.getPayamount()));
                //         payamount = NumberUtil.sub(payamount, records.getPayamount());
                //          orderMasterMapper.updateByPrimaryKey(master);
                if (master.getAlsoypay() <= 0) {
                    if (tag == 2) {
        ...
        Double alsoypay = master.getAlsoypay();
        if ("6".equals(paytype)) {
            Customers cus = customersMapper.selectByPrimaryKey(customerid);//客户信息
            if (!cus.getPasswd().equals(StringUtil.md5(passwd))) {
                throw new AjaxOperationFailException("支付密码错误请重新输入！");
            ...
            //余额
            Double groupbalance = accounts.getGroupbalance();
            if (groupbalance == null || groupbalance <= 0.0) {
                throw new AjaxOperationFailException("余额为0，不能支付");
            }

            Double pay;
            if (groupbalance.doubleValue() >= alsoypay.doubleValue()) { //余额足够
                groupbalance = NumberUtil.sub(groupbalance, alsoypay);
                accounts.setGroupbalance(groupbalance);
                master.setAlsoypay(0.0);
                pay = alsoypay;
            } else {
                master.setAlsoypay(NumberUtil.sub(alsoypay, groupbalance));
                accounts.setGroupbalance(0.0);
                pay = groupbalance;
            }
            //修改用户账户信息
            accountsMapper.updateByPrimaryKey(accounts);
            ...
            logs.setYueamount(accounts.getGroupbalance());
            logs.setOpeation("消费");
            logs.setAmount(-pay);//负数
            logs.setMemo("消费" + pay + "元");
            logs.setIntime(new Date());
            logs.setEmployeeid(employeeService.getLoginUser().getEmployeeid());
            logsMapper.insert(logs);
            if (master.getAlsoypay() <= 0.0) { //余额足够
                master.setAlsoypay(0.0);
                addRecord(master.getAlsoypay(), "5", orderid, customerid, 0, "0", StringUtil.getUUid());
                if (tag == 2) {
                    updateMasterOverAndTables(master, 2, orderSlaveList);
                    updateLinkTableOver(orderMasterList, master);
                } else {
                ...
                master.setAlsoypay(NumberUtil.sub(master.getAlsoypay(), groupbalance));
                addRecord(groupbalance, "5", orderid, customerid, 0, "0", StringUtil.getUUid());
                accounts.setGroupbalance(0.0);
            }
            //发送短信通知
            smsService.sendMessage(cus.getPhone(), "您好，" + cus.getName() + "，本次消费" + payamount + "元，现有余额" + accounts.getGroupbalance() + "元。");
            master.setStatus("1");
        } else if ("0".equals(paytype)) {  //现金支付
            if (payamount >= alsoypay) {
                addRecord(alsoypay, "3", orderid, 0, 0, "0", StringUtil.getUUid());
                master.setAlsoypay(0.0);
                if (tag == 2) {
                    updateMasterOverAndTables(master, 2, orderSlaveList);
                    updateLinkTableOver(orderMasterList, master);
                ...
                master.setStatus("1");
                master.setAlsoypay(NumberUtil.sub(alsoypay, payamount));
                addRecord(payamount, "3", orderid, 0, 0, "0", StringUtil.getUUid());
            }
        } else if ("1".equals(paytype) || "2".equals(paytype)) {
            String payno = StringUtil.getUUid();
            Map<String, Object> map = pay(payno, yuanTofen(alsoypay), code);
            if (map.get("result_code").equals("200") && !map.get("pay_code").equals("PAY_FAIL")) {//成功
                addRecord(alsoypay, paytype, orderid, 0, 0, "0", payno);
                //结账完成
                if (map.get("order_status").equals("PAID")) {
                    master.setAlsoypay(0.0);
                    if (tag == 2) {
            ...
            addRecord(payamount, "4", orderid, 0, 0, "0", StringUtil.getUUid());
            master.setAlsoypay(0.0);
            if (tag == 2) {
                updateMasterOverAndTables(master, 2, orderSlaveList);
                updateLinkTableOver(orderMasterList, master);
            ...
            Double quanMoney = NumberUtil.mul(quannum, quan.getQuanmoney()).doubleValue();
            addRecord(quanMoney, "9", orderid, 0, quannum, "0", StringUtil.getUUid());
            if (quanMoney >= alsoypay) {
                master.setAlsoypay(0.0);
                if (tag == 2) {
                    updateMasterOverAndTables(master, 2, orderSlaveList);
                    updateLinkTableOver(orderMasterList, master);
        ...
        orderMasterMapper.updateByPrimaryKey(master);
        return new ResponseJson("结账成功");
    }

    private PayRecords addRecord(double paymoney, String paytype, int orderid, int customerid,
                                 Integer paynum, String flag, String payno) {
        PayRecords payRecords = new PayRecords();
        Employees employees = employeeService.getLoginUser();
        if (paynum == null) {
            paynum = 0;
        }
        payRecords.setCompanyid(employees.getCompanyid());
        payRecords.setStoreid(employees.getStoreid());
        payRecords.setPayno(payno);
        payRecords.setPaytype(Integer.parseInt(paytype));
        payRecords.setPaytypedesc(paytype + "");
        payRecords.setPayamount(paymoney);
        payRecords.setPaynum(paynum);
        payRecords.setPaytime(new Date());
        payRecords.setReturnamount(0.0);
        payRecords.setReturntime(null);
        payRecords.setMemo("");
        payRecords.setFlag(flag);
        payRecords.setEmployeeid(employees.getEmployeeid());
        payRecords.setOverflag("0");
        payRecords.setStatus("1");
        payRecords.setReturnno("");
        payRecords.setRefundno("");
        payRecords.setStatus("");
    ...
    private void updateLinkTableOver(List<OrderMaster> list, OrderMaster master) {
        Tables tables;
        if (!StringUtil.isListNull(list)) {
            for (OrderMaster orderMaster : list) {
                if (master.getOrderid() != orderMaster.getOrderid()) {
                    tables = tablesMapper.selectByPrimaryKey(orderMaster.getTableid());
                    if (tables != null) {
                        tables.setFlag("0");
                        tablesMapper.updateByPrimaryKey(tables);
                    }
                    orderMaster.setOvertype("0");
                    orderMaster.setStatus("3");
                    orderMaster.setOvertime(new Date());
                    orderMasterMapper.updateByPrimaryKey(orderMaster);
                    //修改点菜为已结账
                    updateOrderSlaveStatus(orderMaster.getOrderid());
                }
            }
        }
    }

    private void updateMasterOverAndTables(OrderMaster master, int ismultiple, List<OrderSlave> orderSlaveList) {
        //修改订单信息
        master.setAlsoypay(0.0);
        master.setOvertype("0");
        master.setOvertime(new Date());
        ...
        //修改点菜为已结账
        Double tradio = master.getDiscountratio();
        String isall = master.getIsall();
        if (tradio == null || tradio == 0.0) {
            tradio = 1.0;
        }
        for (OrderSlave orderSlave: orderSlaveList) {
            orderSlave.setStatus("2");
            if ("1".equals(isall)) {
                orderSlave.setRealamount(NumberUtil.mul(orderSlave.getOrderamount(), tradio));
            } else if ("1".equals(orderSlave.getState())) {
                orderSlave.setRealamount(NumberUtil.mul(orderSlave.getOrderamount(), tradio));
            }
            orderSlaveMapper.updateByPrimaryKey(orderSlave);
        }
    }
}
